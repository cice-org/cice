// Inspired by https://github.com/NiiightmareXD/windows-capture
...
#[derive(snafu::Snafu, Eq, PartialEq, Clone, Debug)]
pub enum Error {
    #[snafu(display("No active window found."))]
    NoActiveWindow,
    #[snafu(display("Failed to find a window with the name: {name}"))]
    NotFound { name: String },
    #[snafu(display("Failed to convert a Windows string from UTF-16"))]
    FailedToConvertWindowsString,
    #[snafu(display("A Windows API call failed: {error}"))]
    WindowsError { error: windows::core::Error },
}

impl From<windows::core::Error> for Error {
    fn from(value: windows::core::Error) -> Self {
        Self::WindowsError { error: value }
    }
}
...
impl Window {
    ...
    pub fn process_id(&self) -> Result<u32, Error> {
        ...
        if id == 0 {
            return Err(Error::WindowsError {
                error: windows::core::Error::from_win32().into(),
            });
        }
    ...
    /// ## DPI Definition
    ///
    /// [Microsoft Documentation:dpi-and-device-independent-pixels](https://learn.microsoft.com/windows/win32/learnwin32/dpi-and-device-independent-pixels)
    #[inline]
    pub fn dpi(&self) -> u32 {
        return unsafe { GetDpiForWindow(self.window) };
    }
